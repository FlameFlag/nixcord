...
describe('validateParsedResults', () => {
  test('throws when either result violates schema', () => {
    ...
    expect(() => validateParsedResults(invalid as unknown as ParsedPluginsResult)).toThrow(/settings/i);
    expect(() => validateParsedResults(valid, invalid as unknown as ParsedPluginsResult)).toThrow(/settings/i);
  });
});
