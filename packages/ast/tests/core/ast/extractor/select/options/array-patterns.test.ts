...
describe('array-patterns', () => {
  ...
  describe('extractOptionsFromArrayFrom', () => {
    ...
    test('handles spread with PropertyAccessExpression', () => {
      const project = createProject();
      const sourceFile = project.createSourceFile(
        'test.ts',
        `
        ...
        const obj = { options: [ ...config.options ] };
        `
      );
      const objLiteral = sourceFile
        .getVariableDeclarationOrThrow('obj')
        .getInitializerIfKindOrThrow(SyntaxKind.ObjectLiteralExpression);
      const arr = objLiteral
        .getPropertyOrThrow('options')
        .asKindOrThrow(SyntaxKind.PropertyAssignment)
        .getInitializerIfKindOrThrow(SyntaxKind.ArrayLiteralExpression);
      const checker = project.getTypeChecker();
      const result = extractOptionsFromObjectArray(arr, checker);
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.values).toEqual(['a', 'b']);
      }
    });
